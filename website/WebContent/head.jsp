...
<%if((String)session.getAttribute("user_name")==(String)session.getAttribute("admin") && (String)session.getAttribute("admin")!=null ) 
{%><div align="right"><a href="adminpage.jsp">Back to admin Panel</a></div>
<%} %>
...
query+=" limit 0,5";
	...
	while(rs.next()){ 
		...
		<center><figure><img oncontextmenu="return false;"  src="Images/<%=imgname%>" align="middle" width="700" height="400"><figcaption> <%if(session.getAttribute("user_name")!=null)
			 {
...
String sql;
int count=0;
sql=("select img_name from images where img_name like '%"+s1+"%' or format like '%"+s1+"%' or img_type like '%"+s1+"%' limit 0,5 ");
...
<%
	...
	String sql;
	int count=0;
	sql=("select img_name from images order by upload_date desc limit 0,5 ");
